  ...
  class BookingService
    ...
    def update_booking?(booking_params, offers)
      ...
      if update_booking_and_room(booking, booking_params, offers)
        check_and_send_update_email(previous_attributes)
        return true
      end
    ...
    def update_booking_and_room(booking, booking_params, offers)
      total_amount = Bookings::BookingCalculator.calculate_total_amount(
        booking_params[:checkin_date], booking_params[:checkout_date],
        @room, offers
      )
      booking_params[:total_amount] = total_amount
    ...
    def check_and_send_update_email(previous_attributes)
      if booking_attributes_changed?(@booking, previous_attributes[:checkin_date], previous_attributes[:checkout_date],
        ...
        Bookings::BookingMailerService.send_booking_update_email(@booking)
      end
    end
  end
end
